...
const DAY_MILLISECONDS = utilities.duration("1d");
const DEFAULT_WRITE_INTERVAL = utilities.duration("3s");
...
const typeColor = utilities.concolor({
  log: "b,black/white",
  info: "b,white/blue",
  warn: "b,black/yellow",
  debug: "b,white/green",
...
const textColor = utilities.concolor({
  log: "white",
  info: "white",
  warn: "b,yellow",
  debug: "b,green",
...
class Logger extends events.EventEmitter {
  ...
  async open() {
    ...
    const fileName = utilities.nowDate() + "-" + this.workerId + ".log";
  ...
  async rotate() {
    ...
    try {
      ...
      for (const fileName of files) {
        if (utilities.fileExt(fileName) !== "log") continue;
  ...
  write(type, s) {
    ...
    if (this.toFile[type]) {
      const msg = utilities.replace(message, "\n", LINE_SEPARATOR);
  ...
  normalizeStack(stack) {
    ...
    let res = utilities.replace(stack, STACK_AT, "");
    if (this.home) res = utilities.replace(res, this.home, "");
    return res;
  }
}
